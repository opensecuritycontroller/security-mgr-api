...
public interface ManagerDeviceApi extends AutoCloseable {
    /**
     * @return true if the manager supports grouping devices, false otherwise
     */
    boolean isDeviceGroupSupported();

    /**
     * Retrieves a device from the security manager by its identifier.
     *
    ...
    /**
     * Retrieves a device from the security manager by its name.
     *
     ...
     * @return a opaque byte array representing the device member configuration
     */
    byte[] getDeviceMemberAdditionalConfiguration(DistributedApplianceInstanceElement dai); // TODO emanoel: why is this needed? seems just like the method above.
